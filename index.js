...
const config = {
  ...
  search_engine:[                     //choose search engine which you use
    ...
    {
      name:"头 条",
      template:"https://so.toutiao.com/search?dvpf=pc&source=sug&keyword=$s"
    },
    {
      name:"谷 歌",
  ...
  sell_info:{
    domain:"blog.ntnas.top",
    price:0,                        //domain price
    mon_unit:"yen sign",              //monetary unit 
    contact:[                         //how to contact you
      {
        type:"envelope",               //contact type ("weixin","qq","telegram plane","envelope" or "phone")
        content:"echoding@163.com"
      }
    ]                        
  },
  lists: [                            //Url list
...
function getFavicon(url){
  if(url.match(/https{0,1}:\/\//)){
    ...
    return "https://icon.horse/icon/" + url.split('//')[1];
  }else{
...
function renderIndex(){
  const footer = el('footer',[],el('div',['class="footer"'],'Powered by' + el('a',['class="ui label"','href="https://ntnas.top:82/typecho"','target="_blank"'],el('i',['class="github icon"'],"") + 'Blog') + ' &copy; Base on ' + el('a',['class="ui label"'],el('i',['class="balance scale icon"'],"") + 'MIT License')));
...
function renderHeader(){
  ...
  return el('header',[],el('div',['id="head"','class="ui inverted vertical masthead center aligned segment"'],(config.hitokoto ? el('div',['id="nav"','class="ui container"'],nav) : "") + el('div',['id="title"','class="ui text container"'],title + (config.search ? input + menu :"") + `${config.selling_ads ? '<div><a id="menubtn" class="red ui icon inverted button"><i class="heart icon"></i>  </a></div>' : ''}`)))
}
...
function renderSeller() {
  ...
  var title = el('h1',['class="ui yellow dividing header"'],el('i',['class="gem outline icon"'],"") + el('div',['class="content"'],config.sell_info.domain + ' '));
...
function renderHTML(index,seller) {
  ...
  <head>
      ...
      <link rel="shortcut icon" href="https://ntnas.top:82/favicon.ico" />
      <link rel="bookmark" href="https://ntnas.top:82/favicon.ico" type="image/x-icon"　/>
  </head>
  <body>
    ${index}
